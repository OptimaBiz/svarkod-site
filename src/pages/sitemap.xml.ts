...
function urlEntry(path: string, lastmod?: Date) {
  ...
  return `<url><loc>${loc}</loc>${lastmod ? `<lastmod>${lastmod.toISOString().slice(0, 10)}</lastmod>` : ''}</url>`;
}
...
export async function GET() {
  ...
  const staticPaths = [
    '/', ...Object.keys(sections).map(section => `/${section}/`), '/documents/', '/organizations/', '/events/', '/ratings/methodology/', '/about/', '/advertising/', '/privacy/', '/ratings/centers/'
  ];
  const urls = [
    ...
    ...events.map(entry => urlEntry(`/events/${entry.id}/`, entry.data.verifiedAt ?? entry.data.startsAt)),
  ];
